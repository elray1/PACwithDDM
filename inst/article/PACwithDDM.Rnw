...
Accelerometers; Conditional Random Field; Hidden Markov Model.
...
Our first classification method is a random forest (\textbf{RF}, \citet{breiman2001RF}).  This is a static discriminative method.  We use the implementation in the {\tt randomForest} package \citep{liaw2002randomForestPackage} for {\tt R} \citep{RCore} with the default options for number of trees, node size, and number of variables considered for each split.  For some of our applications to real data sets, we will present results from support vector machines (\textbf{SVM}) that were previously published in \citet{mannini2013activityrecognition}; this is also a static discriminative classification method.

Our second method is a first-order \textbf{HMM} with one state for each observed activity type and a mixture of Gaussians for the observation distribution:
\begin{eqnarray}
...
The \textbf{HMM} has parameters $\btheta = (\bpi, Q, \bw, \bmu, \bSigma)$, where $\bpi = (\pi_1, \ldots, \pi_S)$, $Q = [q_{r, s}]$, and $\bw$, $\bmu$, and $\bSigma$ contain the parameters for all mixture components.  We estimate $Q$ via maximum likelihood.  We estimate $\bpi_s$ as the observed proportion of the sample with $y_{i, t} = s$; this is not the maximum likelihood estimate, but use of this estimate is a standard procedure to reduce sampling variance of the estimates (e.g., \citet{mcshane2013learningwithTSDependence}).  For the observation distributions, we use {\tt R}'s {\tt mclust} package \citep{Fraley2012mclust} to estimate the mixture weights and the parameters of the Gaussian mixture components.  This package allows for a number of possible restrictions on the parameterizations of the Gaussian component covariance matrices.  It uses an EM algorithm to obtain local maximum likelihood estimates of the Gaussian component parameters, and BIC to select the covariance parameterization and number of components.  Before fitting the model, we apply the Yeo-Johnson transformation \citep{yeo2000powertransform} to each covariate to approximate normality. We use the implementation of this transformation that is available in the {\tt car} package \citep{Fox2011CAR} for {\tt R}. 

%Our third classification method is the discriminative estimation procedure for HMMs described by \citet{mcshane2013learningwithTSDependence}.  This approach combines a static discriminative classifier with the temporal dependence structure of the HMM and can be viewed as adding temporal dependence structures to the random forest, allowing us to compare the static and dynamic approaches based on discriminative models.

Our final classification method is a linear chain \textbf{CRF}.  The model is specified as follows:
\begin{eqnarray*}
pr( \bY _i = \by_i | \bX_i = \bx_i ; \btheta) & = & \frac{1}{Z(\bx_i ; \btheta)} \exp \left\{ \sum_{s = 1}^S \bI_{\{s\}}(y_{i, 1}) \zeta_s \right.  \\
&  & + \sum_{t = 2}^{T_i} \sum_{r = 1}^S \sum_{s = 1}^S \bI_{\{r\}}(y_{i, t - 1}) \bI_{\{s\}}(y_{i, t}) \omega_{r, s}  \\
&  & + \left. \sum_{t = 1}^{T_i} \sum_{s = 1}^S \left( \beta_{s, 0} + \sum_{d = 1}^D \beta_{s, d} x_{i, t, d} \right) \right\}.
\end{eqnarray*}
Here, $Z(\bx_i ; \btheta)$ is a normalizing factor ensuring that the distribution sums to $1$ and $\bI_A(x)$ is the indicator function, taking the value $1$ if $x \in A$ and $0$ otherwise.  More flexible CRF models can be formulated; we use this linear chain CRF specification because it is the model that arises if we condition on $\bX$ in the HMM given by Equations \ref{eqn:HMMInitialState} through \ref{eqn:HMMObsDist} if we fix the number of mixture components $K_s = 1.$  This allows us to compare the relative benefits of using the discriminative and generative approaches in models that account for temporal dependence.  In order to resolve problems with identifiability, we fix $\zeta_S = 0$, $\omega_{S, S} = 0$, and $\beta_{S, d'} = 0 \, \forall d' = 0, \ldots, D$.

Our estimation algorithm for this model employs bagging and boosting.  In the bagging step we generate many different training data sets by drawing observation sequences with replacement from the full set of all observation sequences (note that this same technique is used in the estimation of random forests).  In the final model fit, the coefficient estimates $\zeta_s$, $\omega_{r, s}$, and $\beta_{s, d}$ are the average of the coefficient estimates obtained from separate model fits to each of these bagged data sets.  We use a boosting procedure to obtain these separate model fits.  The boosting step can be interpreted as a random block coordinate ascent algorithm converging to the maximum likelihood parameter estimates based on the given training data set, with early stopping used to reduce overfitting.  We use 10-fold cross-validation estimates of classification performance to select the stopping point for the boosting procedure.  The precise estimation algorithm is in Appendix \ref{sec:appendix:bbparalg}.  Similar estimation strategies for CRFs have been employed in the literature previously (e.g., \citet{smith2007diversityinLOPs, DietterichTrainingCRFviaTreeBoosting}).

\section{Simulation Study}
\label{sec:SimulationStudy}

The objective of the simulation study we describe in this Section is to understand how the performance of the classification methods outlined in Section \ref{sec:Models} depends on the complexity of the distributions for the feature vectors derived from the accelerometer data in each window.  There are many other characteristics of classification problems that likely affect the performance of the methods under consideration, such as the sample size, the dimension of the observed feature vectors, the number of classes, the relative frequencies of each class, the frequency of mislabeled observations in the training data, and so on.  We focus on the complexity of the observation distributions because we believe they are the most useful in helping to explain differences in the performance of discriminative and generative classification methods when applied to real physical activity data.
...
The data $(\by_i, \bx_i) \in \{1, \ldots, S\}^T \times \mathbb{R}^{D \cdot T}$, $i = 1, \ldots, N$, are generated from a first-order HMM as follows:
\begin{eqnarray*}
...
Figure~\ref{fig:simStudyResultsBoxplotsp} summarizes the results of the simulation study.  In the cases with simple observation distributions, when the \textbf{HMM} model is the true data generating model, that method has the best performance.  However, in the cases with complex observation distributions, where the generative \textbf{HMM} model is misspecified, it is outperformed by the \textbf{CRF} method.  The static \textbf{RF} method, which does not account for temporal dependence, is consistently outperformed by the dynamic \textbf{HMM} and \textbf{CRF} models.

\begin{figure}
<<simStudyResultsBoxplotsp, cache = FALSE, fig.height = 5>>=
origsimStudyResults <- simStudyResults
#simStudyResults <- simStudyResults[simStudyResults$fit_method %in% c("Gaussian-mixt-HMM", "BB-Par-CRF", "RF", "Bayes-Rule"), ]
simStudyResults <- simStudyResults[simStudyResults$fit_method %in% c("Gaussian-mixt-HMM", "BB-Par-CRF", "RF"), ]

fit_method_levels <- levels(simStudyResults$fit_method)
fit_method_levels[fit_method_levels == "Gaussian-mixt-HMM"] <- "HMM"
fit_method_levels[fit_method_levels == "BB-Par-CRF"] <- "CRF"
fit_method_levels[fit_method_levels == "Bayes-Rule"] <- "Bayes Rule"
simStudyResults$fit_method <- as.character(simStudyResults$fit_method)
simStudyResults$fit_method[simStudyResults$fit_method == "Gaussian-mixt-HMM"] <- "HMM"
simStudyResults$fit_method[simStudyResults$fit_method == "BB-Par-CRF"] <- "CRF"
simStudyResults$fit_method[simStudyResults$fit_method == "Bayes-Rule"] <- "Bayes Rule"
simStudyResults$fit_method <- factor(simStudyResults$fit_method, levels = fit_method_levels)
...
levels(simStudyResults$bayes_error_rate_high) <- c("Low", "High")
simStudyResults$bayes_error_rate_high_pretty <- simStudyResults$bayes_error_rate_high
...
p <- ggplot(simStudyResults[simStudyResults$bayes_error_rate_high == "Low", ], aes(x = fit_method, y = prop_correct)) +
	geom_point(position = position_jitter(w = 0.2)) +
	geom_boxplot(fill = "white", outlier.colour = NA) +
	facet_grid(~ obs_dist_complex_pretty) +
...
grid.text("Proportion Correct by Classification Method and\nComplexity Level of the Feature Emission Distributions", gp = gpar(fontsize = 16), vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
@
...
# get point estimates
lc_df$pt_est <- K_mat %*% matrix(fixef(results_fit))

# get familywise CIs
fam_CI_obj <- glht(results_fit, linfct = K_mat)
temp <- confint(fam_CI_obj)$confint
lc_df$fam_CI_lb <- temp[, 2]
lc_df$fam_CI_ub <- temp[, 3]

# get individual CIs
lc_df$ind_CI_lb <- NA
lc_df$ind_CI_ub <- NA
for(rowind in seq_len(nrow(lc_df))) {
...
ggplot(summary_figure_df) +
  ...
  theme_bw()
@
\caption{Results from activity type and intensity classification tasks in data from \citet{mannini2013activityrecognition} and \citet{sasaki2016ActivityClassificationOlder}, averaged across the three data sets and two accelerometer locations.  The 95\% confidence intervals are from a linear mixed effects model.}
\end{figure}
...
The figure indicates that the discriminative dynamic model (\textbf{CRF}) improves over the static approach, and the generative dynamic model (\textbf{HMM}) performs worse than the static model or than the discriminative dynamic model.  A mixed model approach to assessing significance found that differences in the proportion correct as small as 1\% were statistically significant \citet{ray2015thesis}.  While the average differences between these approach are relatively small, table \ref{tbl:detailedResults} contains the average percent correct for each model, dataset, and location. For individual instances, the differences between the methods can be larger. Using a different summary of classification performance such as the $F_1$ score did not change these conclusions.
