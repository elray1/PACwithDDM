...
na_vec <- rep(NA, 3 * 2 * (8 * 2 * 15 + 1 * 1 * 15))
...
SasakiFreeLivingTypeResults <- results_df
for(colind in seq_len(5))
	SasakiFreeLivingTypeResults[, colind] <- as.factor(SasakiFreeLivingTypeResults[, colind])

save(SasakiFreeLivingTypeResults, file = paste0(combined_results_dir, "SasakiFreeLivingTypeResults.rdata"))
