...
results_dir <- "C:/Stat/HMM/HMMEnsembles/HMMapplication/Sasaki/intensity/results/freeliving"
...
for(location in c("ankle", "hip", "wrist")) {
	if(identical(location, "ankle")) {
		if(identical(setting, "freeliving")) {
			...
			fit_data <- lapply(fit_data, function(comp) {
				junk <- as.character(comp$y_intensity)
				comp$y_intensity <- factor(junk, levels = c("Sedentary", "Light", "Moderate", "Vigorous", "transition"))
				return(comp)
			})
...
SasakiFreeLivingIntensityResults <- results_df
for(colind in seq_len(5))
	SasakiFreeLivingIntensityResults[, colind] <- as.factor(SasakiFreeLivingIntensityResults[, colind])

save(SasakiFreeLivingIntensityResults, file = paste0(combined_results_dir, "SasakiFreeLivingIntensityResults.rdata"))
