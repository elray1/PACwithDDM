        ...
        for(location in location_levels) {
            if(identical(data_set, "SasakiFreeLiving")) {
                ...
                if(identical(class_var_group, "Type")) {
                    class_var_levels <- c("y_category3", "y_category5")
                } else {
            ...
            for(class_var in class_var_levels) {
                ...
                if(identical(class_var, "y_category4")) {
                    all_fit_methods <- c("L2RegularizedCRF", "unregularizedParametricCRF", "parametricBoostCRF", "RF", "RFHMM", "normalHMM", "SVM")
                } else {
                    all_fit_methods <- c("L2RegularizedCRF", "unregularizedParametricCRF", "parametricBoostCRF", "RF", "RFHMM", "normalHMM")
                }
                
                results_dir <- file.path(results_dir_base, data_set, location, class_var)
                
#    			all_fit_methods <- c("L2RegularizedCRF", "baggedFeatureSubsetGradientTreeBoostCRF", "parametricBoostCRF", "RFCRF", "RFCRFseqbag", "unregularizedParametricCRF", "RF", "RFHMM", "MLRHMM", "normalHMM", "SVM")
                ...
                for(fit_method in all_fit_methods) {
    			    ...
    			    for(subject in seq_len(N)) {
    					if(identical(fit_method, "L2RegularizedCRF")) {
    						results_filename <- paste0("results_FullTrans_subject", subject, ".Rdata")
    					} else if(identical(fit_method, "RF") && class_var %in% c("y_category3", "y_category4", "y_category5")) {
    					...
    					if(identical(class_var, "y_intensity")) {
#    						load(file.path(results_dir_base, data_set, location, class_var, "1stage/notruth", location, fit_method, results_filename))
    					...
    					} else if(identical(fit_method, "L2RegularizedCRF") && identical(class_var_group, "Intensity") && data_set %in% c("SasakiLab", "SasakiFreeLiving")) {
    					...
    					if(fit_method %in% c("baggedFeatureSubsetGradientTreeBoostCRF", "parametricBoostCRF", "RFCRF", "RFCRFseqbag", "gradientTreeBoostCRF", "unregularizedParametricCRF", "SVM", "RFHMM", "MLRHMM", "L2RegularizedCRF")) {
        ...
        results_df$fit_method[results_df$fit_method == "L2RegularizedCRF"] <- "L2-Penalized-CRF"
        ...
        results_df$fit_method <- factor(results_df$fit_method, levels = c("BB-Par-CRF", "BB-Nonpar-CRF", "RF-CRF", "RF-seq-CRF", "Par-CRF", "RF-HMM", "MLR-HMM", "RF", "SVM", "Gaussian-mixt-HMM", "L2-Penalized-CRF"))
