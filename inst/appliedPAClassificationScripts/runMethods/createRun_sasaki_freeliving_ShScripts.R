for(setting in c("freeliving")) {
	for(location in c("ankle", "hip", "wrist")) {
		for(class_var in c("y_category3", "y_category5")) {
			for(fit_method in c("RFCRF", "RFCRFseqbag", "baggedFeatureSubsetGradientTreeBoostCRF", "parametricBoostCRF", "L2RegularizedCRF", "gradientTreeBoostCRF")) {
				...
				if(identical(fit_method, "RFCRF")) {
					...
					cores_req <- "10"
					span_one_host <- FALSE
					mem_req_first <- "150000"
					mem_req_others <- "15000"
					time_req <- "48:00"
					queue_req <- "long"
				}
				
				for(reduced_trans_mat_parameterization in c(TRUE, FALSE)) {
					...
					for(update_trans in update_trans_levels) {
						files_path <- file.path("/home", "er71a", "HMMapplication", "Sasaki", setting, location, class_var, fit_method)
						setwd(files_path)
						...
						for(subject in seq_len(N)) {
							...
							cat(paste0("R CMD BATCH --vanilla \'--args ", subject, " ", setting, " ", location, " ", class_var, " ", fit_method, " ", reduced_trans_mat_parameterization, " ", update_trans,
												 "\' /home/er71a/HMMapplication/Sasaki/sasakiLOSOcrossval.R ", Routbasefilename, subject, ".Rout"),
									file = filename, append = TRUE)
