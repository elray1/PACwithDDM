...
#  for(location in c("ankle", "hip", "wrist")) {
  for(location in c("ankle", "wrist")) {
    ...
    plot_file_path <- paste0("C:/Stat/HMM/HMMEnsembles/HMMapplication/Sasaki/freeliving/plots/initial/unprocessed/", location, "/unprocessed_data_classification_plot_behavior_subj", subj, ".png")
    ...
    unprocessed_data <- combine_free_living_actigraph_and_DO_files(actigraph_file_path, DO_file_path)
...
#    recorded_plots <- make_signal_classification_plots(signal_vars = c("vm", "x", "y", "z"), class_vars = c("behavior", "behavior_mod_2"), data = unprocessed_data,
		recorded_plots <- make_signal_classification_plots(signal_vars = c("vm"), class_vars = c("behavior"), data = unprocessed_data,
																											 sampling_freq = 80, subsample_rate = 10, panel_minutes = 30, main_title = plot_main_title,
