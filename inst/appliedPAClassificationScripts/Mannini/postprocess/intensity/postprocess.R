...
results_dir <- "C:/Stat/HMM/HMMEnsembles/HMMapplication/Mannini/intensity/results"
combined_results_dir <- "C:/Stat/HMM/HMMEnsembles/rayThesis/data/"

N <- 33

na_vec <- NA
results_df <- data.frame(location = na_vec, class_var = na_vec, fit_method = na_vec, case = na_vec, subject = na_vec,
...
ManniniIntensityResults <- results_df
for(colind in seq_len(5))
	ManniniIntensityResults[, colind] <- as.factor(ManniniIntensityResults[, colind])

save(ManniniIntensityResults, file = paste0(combined_results_dir, "ManniniIntensityResults.rdata"))
