...
void sample_int_without_replaceOLD(int min, int max, int N, int *result) {
	int i, j, K, temp, offset;
	
	K = max - min + 1;
//	Rprintf("In sample_int_without_replace, min = %i, max = %i, K = %i, N = %i\n", min, max, K, N);
	if(N >= 1) {
		if(N == K) {
			for(i = 0; i < N; i++)
				*(result + i) = i;
			...
			for(i = 0; i < N; i++) {
				...
				//			Rprintf("calling rmultinom with probs: ");
				//			for(j = 0; j < K; j++) {
				//				Rprintf("%lf, ", probs[j]);
				//			}
				rmultinom(1, (double*)&probs, K, (int*)&obs);
//				Rprintf("\nobs multinom vector is: ");
//				for(j = 0; j < K; j++) {
//					Rprintf("%i, ", obs[j]);
//				}
				//			Rprintf("\nexited rmultinom\n");
				...
				// which slot is the Kth empty slot?
				offset = 0;
				j = 0;
				while(j <= temp + offset) {
					if(prev[j])
				...
				// store the results
				prev[temp + offset] = 1;
				*(result + i) = temp + offset + min;
//				Rprintf("result %i is %i\n", i, *(result + i));
				obs[temp + offset] = 1;
